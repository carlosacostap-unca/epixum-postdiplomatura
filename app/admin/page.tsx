...
import { getAllCourses, getUsers } from "@/lib/data";
import { PageHeader, StatCard } from "@/components/ui";

export default async function AdminDashboardPage() {
  const [courses, users] = await Promise.all([getAllCourses(), getUsers()]);
  ...
  const teachers = users.filter((user) => user.role === "docente").length;
  const students = users.filter((user) => user.role === "estudiante").length;
  ...
  return (
    <div className="p-6 md:p-10 xl:p-12">
      ...
      <section aria-label="Resumen operativo" className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard href="/admin/users?role=docente" label="Docentes" value={teachers} icon="co_present" tone="info" description="Con acceso docente" />
