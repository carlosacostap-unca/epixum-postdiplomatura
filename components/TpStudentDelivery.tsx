...
export default function TpStudentDelivery({ assignmentId, courseId, delivery, dueDate }: { assignmentId: string; courseId: string; delivery: Delivery | null; dueDate?: string }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setIsEditing(false);
      setSelectedFiles([]);
      notify({ title: delivery ? "Entrega actualizada" : "Entrega enviada", description: mode === "url" ? "El enlace quedó guardado correctamente." : "Los archivos quedaron guardados correctamente.", tone: "success" });
      router.refresh();
    } catch (caught) {
  ...
  return <div className="space-y-6">
      ...
      <Card>
        <CardContent className="space-y-5">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-3"><Badge tone="success">Entrega enviada</Badge>{delivery.status === "published" && <Badge tone="info">Evaluada</Badge>}</div>
              <p className="mt-3 text-sm text-[var(--color-on-surface-variant)]">Enviada <FormattedDate date={delivery.created} showTime /></p>
            </div>
            {!isPastDue && <Button variant="secondary" leadingIcon={<span className="material-symbols-outlined text-lg">edit</span>} onClick={() => setIsEditing(true)}>Actualizar entrega</Button>}
          </div>
          {existingSubmission.type === "url" ? (
            ...
            <div className="space-y-2">
              ...
              {existingSubmission.files.map((file, index) => <Button key={`${file.url}-${index}`} variant="secondary" isPending={downloadingIndex === index} pendingLabel="Preparando…" leadingIcon={<span className="material-symbols-outlined text-lg">download</span>} onClick={() => download(index, file.name)} className="w-full justify-start"><span className="truncate">{file.name}</span></Button>)}
            </div>
          )}
          {isPastDue && <p className="flex items-center gap-2 text-sm text-[var(--color-on-surface-variant)]"><span className="material-symbols-outlined text-lg" aria-hidden="true">lock</span>El plazo terminó. Tu entrega se conserva, pero ya no puede modificarse.</p>}
        </CardContent>
      </Card>
    )}
    ...
    {isEditing && !isPastDue && (
      <Card>
        <CardContent className="space-y-5">
          <div><h3 className="font-headline text-xl font-bold">{delivery ? "Actualizar entrega" : "Enviar entrega"}</h3><p className="mt-1 text-sm text-[var(--color-on-surface-variant)]">Elegí archivos o compartí una URL. La nueva opción reemplazará la entrega actual.</p></div>
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <Button isPending={loading} pendingLabel="Enviando…" disabled={mode === "files" ? selectedFiles.length === 0 : deliveryUrl.trim().length === 0} leadingIcon={<span className="material-symbols-outlined text-lg">send</span>} onClick={submit}>Enviar entrega</Button>
          </div>
        </CardContent>
      </Card>
    )}
